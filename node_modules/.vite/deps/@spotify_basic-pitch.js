...
// node_modules/@spotify/basic-pitch/esm/inference.js
import * as tf from "@tensorflow/tfjs";
var OUTPUT_TO_TENSOR_NAME = {
  contours: "Identity",
  onsets: "Identity_2",
  frames: "Identity_1"
};
...
var FFT_HOP = 256;
var ANNOTATIONS_FPS = Math.floor(AUDIO_SAMPLE_RATE / FFT_HOP);
var AUDIO_WINDOW_LENGTH_SECONDS = 2;
var AUDIO_N_SAMPLES = AUDIO_SAMPLE_RATE * AUDIO_WINDOW_LENGTH_SECONDS - FFT_HOP;
var N_OVERLAPPING_FRAMES = 30;
var N_OVERLAP_OVER_2 = Math.floor(N_OVERLAPPING_FRAMES / 2);
var OVERLAP_LENGTH_FRAMES = N_OVERLAPPING_FRAMES * FFT_HOP;
var HOP_SIZE = AUDIO_N_SAMPLES - OVERLAP_LENGTH_FRAMES;
var BasicPitch = class {
  constructor(modelOrModelPath) {
    ...
    this.model = typeof modelOrModelPath === "string" ? tf.loadGraphModel(modelOrModelPath) : modelOrModelPath;
  }
  ...
  async evaluateModel(resampledBuffer, onComplete, percentCallback) {
    ...
    if (resampledBuffer instanceof Float32Array) {
      ...
      if (resampledBuffer.sampleRate !== AUDIO_SAMPLE_RATE) {
        throw new Error(`Input audio buffer is not at correct sample rate! Is ${resampledBuffer.sampleRate}. Should be ${AUDIO_SAMPLE_RATE}`);
      }
      if (resampledBuffer.numberOfChannels !== NUM_CHANNELS) {
        throw new Error(`Input audio buffer is not mono! Number of channels is ${resampledBuffer.numberOfChannels}. Should be ${NUM_CHANNELS}`);
      }
...
// node_modules/@spotify/basic-pitch/esm/toMidi.js
var import_midi = __toESM(require_Midi());
var MIDI_OFFSET = 21;
var AUDIO_SAMPLE_RATE2 = 22050;
var AUDIO_WINDOW_LENGTH = 2;
var FFT_HOP2 = 256;
var ANNOTATIONS_FPS2 = Math.floor(AUDIO_SAMPLE_RATE2 / FFT_HOP2);
var ANNOT_N_FRAMES = ANNOTATIONS_FPS2 * AUDIO_WINDOW_LENGTH;
var AUDIO_N_SAMPLES2 = AUDIO_SAMPLE_RATE2 * AUDIO_WINDOW_LENGTH - FFT_HOP2;
var WINDOW_OFFSET = FFT_HOP2 / AUDIO_SAMPLE_RATE2 * (ANNOT_N_FRAMES - AUDIO_N_SAMPLES2 / FFT_HOP2) + 18e-4;
var MAX_FREQ_IDX = 87;
var CONTOURS_BINS_PER_SEMITONE = 3;
var ANNOTATIONS_BASE_FREQUENCY = 27.5;
var ANNOTATIONS_N_SEMITONES = 88;
var N_FREQ_BINS_CONTOURS = ANNOTATIONS_N_SEMITONES * CONTOURS_BINS_PER_SEMITONE;
var hzToMidi = (hz) => 12 * (Math.log2(hz) - Math.log2(440)) + 69;
var midiToHz = (midi) => 440 * 2 ** ((midi - 69) / 12);
var modelFrameToTime = (frame) => frame * FFT_HOP2 / AUDIO_SAMPLE_RATE2 - WINDOW_OFFSET * Math.floor(frame / ANNOT_N_FRAMES);
function argMax(arr) {
  return arr.length === 0 ? null : arr.reduce((maxIndex, currentValue, index) => arr[maxIndex] > currentValue ? maxIndex : index, -1);
}
var argMaxAxis1 = (arr) => arr.map((row) => argMax(row));
...
var gaussian = (M, std) => Array.from(Array(M).keys()).map((n) => Math.exp(-1 * (n - (M - 1) / 2) ** 2 / (2 * std ** 2)));
var midiPitchToContourBin = (pitchMidi) => 12 * CONTOURS_BINS_PER_SEMITONE * Math.log2(midiToHz(pitchMidi) / ANNOTATIONS_BASE_FREQUENCY);
function addPitchBendsToNoteEvents(contours, notes, nBinsTolerance = 25) {
  ...
  return notes.map((note) => {
    ...
    const freqGuassianSubMatrix = freqGaussian.slice(Math.max(0, nBinsTolerance - freqIdx), windowLength - Math.max(0, freqIdx - (N_FREQ_BINS_CONTOURS - nBinsTolerance - 1)));
    const pitchBendSubmatrix = contours.slice(note.startFrame, note.startFrame + note.durationFrames).map((d) => d.slice(freqStartIdx, freqEndIdx).map((v, col) => v * freqGuassianSubMatrix[col]));
...
var noteFramesToTime = (notes) => notes.map((note) => {
  return {
...
//# sourceMappingURL=@spotify_basic-pitch.js.map
